...
public class GaleShapley {
    ...
    // 생성자
    public GaleShapley(Map<String, Man> m, Map<String, Woman> w) {
                ...
                for (String w : free.getPreferences()) {  // 그 남자의 선호도 목록을 순회
                    ...
                    if (!free.isEngaged() && !free.getProposed()
                        ...
                        if (woman.getPartner() == null) {  // 여자가 아직 매칭되지 않았으면
                            woman.setPartner(free.getName());  // 여자와 남자를 매칭
                            free.setEngaged(true);  // 남자의 상태를 매칭됨으로 변경
                            engagedCount++;  // 매칭된 커플 수 증가
                        } else {  // 여자가 이미 매칭되어 있으면
                            Man currentPartner = men.get(woman.getPartner());  // 현재 파트너를 찾음
    ...
    // private로 변환
    public void firstAcceptOrReject() {
        for (Woman woman : women.values()) {
